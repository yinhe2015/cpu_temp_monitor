...
def 处理温度(名称: str, 温度: float, 需要降频: bool = True):
    if 温度 > 错误温度:
        ...
        日志.警告(f'{名称} 温度{温度:.2f}°C > {严重降频温度=}°C{f', 降频{严重降频幅度}%' if 自动降频 else ''}')
        if 需要降频:
            减少xpp(严重降频幅度)
    elif 温度 > 中度降频温度:
        日志.警告(f'{名称} 温度{温度:.2f}°C > {中度降频温度=}°C{f', 降频{中度降频幅度}%' if 自动降频 else ''}')
        if 需要降频:
            减少xpp(中度降频幅度)
    elif 温度 > 轻度降频温度:
        日志.警告(f'{名称} 温度{温度:.2f}°C > {轻度降频温度=}°C{f', 降频{轻度降频幅度}%' if 自动降频 else ''}')
        if 需要降频:
            减少xpp(轻度降频幅度)
    elif 温度 > 警告温度:
...
def 警告与降频处理(主温度: float, 温度列表: list[float]):
    最热温度 = max(温度列表)
    最热核心 = [核心 for 核心, 温度 in enumerate(温度列表, 1) if 温度 == 最热温度]
    最热核心字符串 = '核心' + (','.join([f'{核心}' for 核心 in 最热核心]))
